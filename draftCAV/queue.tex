...
\vspace{-1.5mm}
The typical abstract implementation of a concurrent queue, denoted as $AbsQ_0$, maintains a sequence of values, the enqueue method adds a value atomically to the beginning of the sequence, and the dequeue method removes a value from the end of the sequence (if any, otherwise it returns {\tt EMPTY}). Both methods have a fixed linearization point when the update of the shared sequence happens. %~\footnote{For $\<Methods>=\{enq,deq\}$, the alphabet of $AbsQ_0$ is $C\cup R\cup Lin$.}. 
