...
simulated by $AbsQ_0$, even though they refine $AbsQ_0$, since the order in
which their enqueues are linearized to form $AbsQ_0$’s sequence is not
...
linearizations of their enqueues, it simulates many more queue implementations.
In fact, $AbsQ$ simulates all queue implementations of which we are aware that
are not simulated by $AbsQ_0$, including {\sc hwq}, The Baskets
Queue~\cite{DBLP:conf/opodis/HoffmanSS07}, The Linked Concurrent Ring Queue
({\sc lcrq})~\cite{DBLP:conf/ppopp/MorrisonA13}, and The Time-Stamped
Queue~\cite{DBLP:conf/popl/DoddsHK15}.
