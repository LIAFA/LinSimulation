...
A \emph{labeled transition system} (LTS) $A=(Q,\Sigma, q_0, \delta)$ over the 
possibly-infinite alphabet $\Sigma$ is a possibly-infinite set $Q$ of states with
initial state $q_0 \in Q$, and a transition relation $\delta \subseteq Q \times @S \times
...
$\rho = s_0, e_0,s_1\ldots e_{k-1},s_k$ for some $k>0$ such that $s_0 = q_0$ and $\delta(s_i, e_i, s_{i+1})$
...
say $L_1$ refines $L_2$ if{f} every computation of every programusing $L_1$ would also be possible were $L_2$ used instead. As shown in~\citet{DBLP:conf/popl/BouajjaniEEH15}
this can be equivalently stated as history set inclusion between the two libraries.

\begin{dfn}
Let $L_1$ and $L_2$ be two libraries over a common alphabet $\Sigma$~\footnote{The two libraries could be defined over different alphabets as long as they share the same set of call/return actions.}. 
...
alternative characterization of library conformance which implies 
refinement~\cite{journals/tcs/FilipovicORY10}. 
The two criteria coincide when the reference implementation is atomic~\cite{DBLP:conf/popl/BouajjaniEEH15}.
Linearizability~\cite{journals/toplas/HerlihyW90} is defined by the weakening order $\sqsubseteq$. A 
history $h_1$ is \emph{linearizable} w.r.t.~a library $L_2$ if{f} there exists a sequential history
...
\begin{theorem}[\cite{DBLP:conf/popl/BouajjaniEEH15}]
  $L_1 \sqsubseteq L_2$ if{f} $L_1$ refines $L_2$,
  if $L_2$ is atomic.

\end{theorem}

The weakening order w.r.t. sequential histories can be also defined  in terms of ~\emph{linearization points}. Informally, a linearization point 
of an operation in an execution is a point in time where the operation is conceptually effectuated. It corresponds to a transition in that
...
An operation $k$ is called \emph{effectuated} in a complete LP-annotation $\hat{h}$ iff  $\hat{h}$ contains $lin(\_,\_,k)$. 

It has been shown that $h_1 \sqsubseteq h_2$ for a sequential history $h_2$ if{f} there exists a complete LP-annotation $\hat{h_1}$ such that  
...
\begin{dfn}
Let $L_1=(Q_1,\Sigma, q_0^1, \delta_1)$ and $L_2=(Q_2,\Sigma, q_0^2, \delta_2)$ be two libraries over a common alphabet $\Sigma$. A relation $\mathit{fs} \subseteq Q_{1} \times Q_{2}$ is called a \emph{forward simulation} from $L_1$ to $L_2$ iff the following holds:
\begin{itemize}
\item[(i)] $\mathit{fs}[q_0^1] = \{q_0^2 \}$
...
\item[(ii-c)] If $(s,e,s') \in \delta_1$, for some $e \in \Sigma\setminus (C\cup R)$ and $u \in \mathit{fs}[s]$, then there exists $u' \in \mathit{fs}[s']$ such that $u \xrightarrow{\sigma} u'$ and $@s_i\in \Sigma\setminus(C\cup R)$, for each $0\leq i<|@s|$.
...
Simulation relations are used to prove refinement among libraries. The following lemma shows their soundness:
\begin{lem}
Let $L_1$ and $L_2$ be two libraries over a common alphabet $\Sigma$. If there exists a forward simulation from $L_1$ to $L_2$, then $L_1$ refines $L_2$. Moreover, if $L_2$ is deterministic and $L_1$ refines $L_2$, then there exists a forward simulation from $L_1$ to $L_2$.
\end{lem}
\begin{proof}
