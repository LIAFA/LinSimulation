...
abstracting away the details of any particular programming language irrelevant to
our development.
...
Q$. The $i$th symbol of a sequence $@t \in @S^*$ is denoted $@t_i$, and $\epsilon$ denotes the empty
sequence.
An \emph{execution} of $A$ is an alternating sequence of states and transition labels (also called actions)
...
receive \emph{arguments} and yield \emph{return values} upon completion.
...
We may omit the second field from a call/return action for methods that have no arguments or return values.
...
e.g., return actions correspond to previous call actions.
...
%, which for lack of 
%space are delegated to Appendix~\ref{app:prelim}. 
An operation $k$ is called \emph{completed} in a trace $\tau$ when
...
%We assume that $H(L)=\overline{H(L)}$ for
...
%given two libraries
%$L_1$ and $L_2$ implementing the methods of some concurrent object, 
we
